...
if ( ! function_exists( 'ot_type_theme_options_ui' ) ) {
	...
	function ot_type_theme_options_ui() {
		...
		echo '<h4>' . esc_html__( 'Warning!', 'sidebar-menu' ) . '</h4>';

		/* translators: %s: link to theme options */
		$string = esc_html__( 'Go to the %s page if you want to save data, this page is for adding settings.', 'sidebar-menu' );
		echo '<p class="warning">' . sprintf( $string, '<a href="' . esc_url_raw( get_admin_url( $blog_id, apply_filters( 'ot_theme_options_parent_slug', 'themes.php' ) ) . '?page=' . apply_filters( 'ot_theme_options_menu_slug', 'ot-theme-options' ) ) . '"><code>' . esc_html__( 'Appearance->Theme Options', 'sidebar-menu' ) . '</code></a>' ) . '</p>'; // phpcs:ignore

		/* translators: %s: link to documentation */
		$string = esc_html__( 'If you\'re unsure or not completely positive that you should be editing these settings, you should read the %s first.', 'sidebar-menu' );
		echo '<p class="warning">' . sprintf( $string, '<a href="' . esc_url_raw( get_admin_url( $blog_id, 'admin.php?page=ot-documentation' ) ) . '"><code>' . esc_html__( 'OptionTree->Documentation', 'sidebar-menu' ) . '</code></a>' ) . '</p>'; // phpcs:ignore

		echo '<h4>' . esc_html__( 'Things could break or be improperly displayed to the end-user if you do one of the following:', 'sidebar-menu' ) . '</h4>';
		echo '<p class="warning">' . esc_html__( 'Give two sections the same ID, give two settings the same ID, give two contextual help content areas the same ID, don\'t create any settings, or have a section at the end of the settings list.', 'sidebar-menu' ) . '</p>';
		...
		echo '<p>' . esc_html__( 'All of the settings can be sorted and rearranged to your liking with Drag & Drop. Don\'t worry about the order in which you create your settings, you can always reorder them.', 'sidebar-menu' ) . '</p>';
		...
		// Buttons.
		echo '<a href="javascript:void(0);" class="option-tree-section-add option-tree-ui-button button hug-left">' . esc_html__( 'Add Section', 'sidebar-menu' ) . '</a>';
		echo '<a href="javascript:void(0);" class="option-tree-setting-add option-tree-ui-button button">' . esc_html__( 'Add Setting', 'sidebar-menu' ) . '</a>';
		echo '<button class="option-tree-ui-button button button-primary right hug-right">' . esc_html__( 'Save Changes', 'sidebar-menu' ) . '</button>';

		// Sidebar textarea.
		echo '
		<div class="format-setting-label" id="contextual-help-label">
			<h3 class="label">' . esc_html__( 'Contextual Help', 'sidebar-menu' ) . '</h3>
		</div>
		<div class="format-settings" id="contextual-help-setting">
			<div class="format-setting type-textarea no-desc">
				<div class="description"><strong>' . esc_html__( 'Contextual Help Sidebar', 'sidebar-menu' ) . '</strong>: ' . esc_html__( 'If you decide to add contextual help to the Theme Option page, enter the optional "Sidebar" HTML here. This would be an extremely useful place to add links to your themes documentation or support forum. Only after you\'ve added some content below will this display to the user.', 'sidebar-menu' ) . '</div>
		...
		echo '<a href="javascript:void(0);" class="option-tree-help-add option-tree-ui-button button hug-left">' . esc_html__( 'Add Contextual Help Content', 'sidebar-menu' ) . '</a>';
		echo '<button class="option-tree-ui-button button button-primary right hug-right">' . esc_html__( 'Save Changes', 'sidebar-menu' ) . '</button>';
...
if ( ! function_exists( 'ot_type_import_settings' ) ) {
	...
	function ot_type_import_settings() {
		...
		echo '<p>' . esc_html__( 'To import your Settings copy and paste what appears to be a random string of alpha numeric characters into this textarea and press the "Import Settings" button.', 'sidebar-menu' ) . '</p>';

		echo '<button class="option-tree-ui-button button button-primary right hug-right">' . esc_html__( 'Import Settings', 'sidebar-menu' ) . '</button>';
...
if ( ! function_exists( 'ot_type_import_data' ) ) {
	...
	function ot_type_import_data() {
		...
		if ( OT_SHOW_SETTINGS_IMPORT ) {
			echo '<p>' . esc_html__( 'Only after you\'ve imported the Settings should you try and update your Theme Options.', 'sidebar-menu' ) . '</p>';
		}

		echo '<p>' . esc_html__( 'To import your Theme Options copy and paste what appears to be a random string of alpha numeric characters into this textarea and press the "Import Theme Options" button.', 'sidebar-menu' ) . '</p>';

		echo '<button class="option-tree-ui-button button button-primary right hug-right">' . esc_html__( 'Import Theme Options', 'sidebar-menu' ) . '</button>';
...
if ( ! function_exists( 'ot_type_import_layouts' ) ) {
	...
	function ot_type_import_layouts() {
		...
		if ( OT_SHOW_SETTINGS_IMPORT ) {
			echo '<p>' . esc_html__( 'Only after you\'ve imported the Settings should you try and update your Layouts.', 'sidebar-menu' ) . '</p>';
		}
		...
		echo '<button class="option-tree-ui-button button button-primary right hug-right">' . esc_html__( 'Import Layouts', 'sidebar-menu' ) . '</button>';
...
if ( ! function_exists( 'ot_type_export_settings_file' ) ) {
	...
	function ot_type_export_settings_file() {
		...
		echo '<p>' . sprintf( $string, '<code>theme-options.php</code>', '<a href="http://codex.wordpress.org/I18n_for_WordPress_Developers" target="_blank">I18n</a>', '<a href="' . get_admin_url( $blog_id, 'admin.php?page=ot-documentation#section_theme_mode' ) . '">' . esc_html__( 'Theme Mode', 'sidebar-menu' ) . '</a>' ) . '</p>'; // phpcs:ignore
		...
		echo '<button class="option-tree-ui-button button button-primary hug-left">' . esc_html__( 'Export Settings File', 'sidebar-menu' ) . '</button>';
...
if ( ! function_exists( 'ot_type_export_settings' ) ) {
	...
	function ot_type_export_settings() {
		...
		$string = esc_html__( 'Export your Settings by highlighting this text and doing a copy/paste into a blank .txt file. Then save the file for importing into another install of WordPress later. Alternatively, you could just paste it into the %1$s %1$s textarea on another web site.', 'sidebar-menu' );
		echo '<p>' . sprintf( $string, '<code>' . esc_html__( 'OptionTree->Settings->Import', 'sidebar-menu' ) . '</code>', '<code>' . esc_html__( 'Settings', 'sidebar-menu' ) . '</code>' ) . '</p>'; // phpcs:ignore
...
if ( ! function_exists( 'ot_type_export_data' ) ) {
	...
	function ot_type_export_data() {
		...
		$string = esc_html__( 'Export your Theme Options data by highlighting this text and doing a copy/paste into a blank .txt file. Then save the file for importing into another install of WordPress later. Alternatively, you could just paste it into the %1$s %2$s textarea on another web site.', 'sidebar-menu' );
		echo '<p>' . sprintf( $string, '<code>' . esc_html__( 'OptionTree->Settings->Import', 'sidebar-menu' ) . '</code>', '<code>' . esc_html__( 'Theme Options', 'sidebar-menu' ) . '</code>' ) . '</p>'; // phpcs:ignore
...
if ( ! function_exists( 'ot_type_export_layouts' ) ) {
	...
	function ot_type_export_layouts() {
		...
		$string = esc_html__( 'Export your Layouts by highlighting this text and doing a copy/paste into a blank .txt file. Then save the file for importing into another install of WordPress later. Alternatively, you could just paste it into the %1$s %2$s textarea on another web site.', 'sidebar-menu' );
		echo '<p>' . sprintf( $string, '<code>' . esc_html__( 'OptionTree->Settings->Import', 'sidebar-menu' ) . '</code>', '<code>' . esc_html__( 'Layouts', 'sidebar-menu' ) . '</code>' ) . '</p>'; // phpcs:ignore
...
if ( ! function_exists( 'ot_type_modify_layouts' ) ) {
	...
	function ot_type_modify_layouts() {
		...
		echo '<p>' . esc_html__( 'To add a new layout enter a unique lower case alphanumeric string (dashes allowed) in the text field and click "Save Layouts".', 'sidebar-menu' ) . '</p>';
		echo '<p>' . esc_html__( 'As well, you can activate, remove, and drag & drop the order; all situations require you to click "Save Layouts" for the changes to be applied.', 'sidebar-menu' ) . '</p>';
		...
		if ( OT_SHOW_DOCS ) {
			...
			$string = esc_html__( 'Visit %s to see a more in-depth description of what layouts are and how to use them.', 'sidebar-menu' );
			echo '<p>' . sprintf( $string, '<code>' . esc_html__( 'OptionTree->Documentation->Layouts Overview', 'sidebar-menu' ) . '</code>' ) . '</p>'; // phpcs:ignore
		}
		...
		echo '<button class="option-tree-ui-button button button-primary right hug-right">' . esc_html__( 'Save Layouts', 'sidebar-menu' ) . '</button>';
