			...
			foreach ( (array) $this->options as $option ) {
				...
				foreach ( (array) $this->get_pages( $option ) as $page ) {
					...
					if ( ! isset( $page['hidden_page'] ) && $screen->id === $this->page_hook[ $page['id'] ] ) {
						...
						$link = '<a href="https://wordpress.org/plugins/option-tree/" target="_blank">' . esc_html__( 'OptionTree', 'sidebar-menu' ) . '</a>';
						...
						if ( $show_buttons ) {
							...
							echo '<button type="submit" class="option-tree-ui-button button button-secondary left reset-settings" title="' . esc_html__( 'Reset Options', 'sidebar-menu' ) . '">' . esc_html__( 'Reset Options', 'sidebar-menu' ) . '</button>';
